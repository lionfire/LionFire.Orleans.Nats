...
namespace LionFire.Orleans_.Storage
{
    /// <summary>
    /// Logging codes used by <see cref="NatsGrainStorage"/>.
    /// </summary>
    ...
    {
        public static NatsGrainStorage Create(IServiceProvider services, string name)
        {
            var optionsMonitor = services.GetRequiredService<IOptionsMonitor<NatsGrainStorageOptions>>();
            var clusterOptions = services.GetProviderClusterOptions(name);
            return ActivatorUtilities.CreateInstance<NatsGrainStorage>(services, Options.Create(optionsMonitor.Get(name)), name, clusterOptions);
        }
    }
    ...
    public class NatsGrainStorage : IGrainStorage, ILifecycleParticipant<ISiloLifecycle>
    {
        ...
        public NatsGrainStorage(
            ...
            IOptions<NatsGrainStorageOptions> options,
        ...
        {
            lifecycle.Subscribe(OptionFormattingUtilities.Name<NatsGrainStorage>(this.name), this.options.InitStage, Init, Close);
        }
        ...
        private static NatsGrainKey GrainIdAndExtensionAsString(GrainId grainId)
        {
            string keyExt;
            if (grainId.TryGetGuidKey(out var guid, out keyExt))
            {
                return new NatsGrainKey(guid, keyExt);
            }

            if (grainId.TryGetIntegerKey(out var integer, out keyExt))
            {
                return new NatsGrainKey(integer, keyExt);
            }

            return new NatsGrainKey(grainId.Key.ToString());
        }
